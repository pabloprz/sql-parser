...
namespace hsql {
    ...
    Expr *Expr::makeOpBinaryDOW(Expr *expr1, DayOfTheWeek dow) {
        Expr *e = new Expr(kExprOperator);
        e->opType = kOpIsDayOfTheWeek;
        e->expr = expr1;
        e->expr2 = new Expr(kExprLiteralDayOfTheWeek);
        e->expr2->dayOfTheWeek = dow;
        return e;
    }
    ...
    Expr *Expr::makeFunctionRef(char *func_name, Expr *intValue, DatetimeField datetime) {
        Expr *e = new Expr(kExprFunctionRef);
        e->name = func_name;

        auto *vect = new std::vector<Expr *>();
        Expr *interval = new Expr(kExprLiteralInterval);
        interval->ival = intValue->ival;
        interval->datetimeField = datetime;
